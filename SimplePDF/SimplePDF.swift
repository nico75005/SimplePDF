...
//  Updated by Nico75005
//  Copyright © 2018 Nutchaphon Rewik. All rights reserved.
//
...
open class SimplePDF {
	...
	fileprivate func drawTextInCell(_ rect: CGRect, text: NSString, alignment: ContentAlignment, font: UIFont, textColor:UIColor) {
		...
		let textRect: CGRect = { () -> CGRect in
			let textSize = text.size(withAttributes: attributes)
			let y = rect.midY - textSize.height / 2
			let width = min(rect.width, textSize.width)
			let size = CGSize(width: width, height: textSize.height)

			switch alignment {
			case .left:
				return CGRect(origin: CGPoint(x: rect.minX, y: y), size: size)
			case .center:
				return CGRect(center: rect.center, size: size)
			case .right:
				let x = max(rect.minX, rect.maxX - textSize.width)
